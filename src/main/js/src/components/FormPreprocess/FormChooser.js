    ...
    <MembershipContext.Consumer>
      {({ setCurrentFormId }) => (
        <div style={styles}>
          ...
          <button type="button" onClick={fetchExistingForm(setCurrentFormId)} className="btn btn-primary"> Continue Existing Application </button>
          <button type="button" onClick={setCurrentFormId(newForm_tempId)} className="btn btn-primary"> Start New Application </button>
        </div>
      )}
    </MembershipContext.Consumer>
  )
