...
const FormikStepper = ({ step, setStep, children, ...props }) => {
  ...
  const { currentUser, currentFormId, setCurrentFormId } = useContext(
    MembershipContext
  );
    ...
    switch (step) {
      ...
      case 0:
        if (values.representative.marketing.sameAsCompany) {
          assignContactData(
            values.representative.marketing,
            values.representative.company
          );
        }
        if (values.representative.accounting.sameAsCompany) {
          assignContactData(
            values.representative.accounting,
            values.representative.company
          );
        }
